...
client = OpenAI(
    api_key=api_key,
    base_url="https://open.bigmodel.cn/api/paas/v4",  # DeepSeek API 的基地址
)

# 定义提示词
prompt = """请帮我用 HTML 生成一个五子棋游戏，所有代码都保存在一个 HTML 中。包含悔棋、重新开始的功能 """
# %%
try:
    # 调用 DeepSeek Chat API
    response = client.chat.completions.create(
        model="glm-4.6",  # 或 DeepSeek 提供的其他模型名称
        messages=[
